...
class CveFixTask(AsyncTask):
        ...
        command = self.command % json.dumps(
            {
                ...
                "fix_type": task_info.get("fix_type"),
    ...
    def on_success(self, retval, task_id, args, kwargs):
        ...
        if exit_code == ShellExitCode.SUCCESS:
            ...
            result["rpms"] = [
                {"available_rpm": rpm.get("available_rpm"), "result": TaskStatus.FAIL, "log": stderr}
                for rpm in task_info.get("rpms")
            ]
    ...
    def on_failure(self, exc, task_id, args, kwargs, einfo):
        ...
        result = self._request_body(host_info, task_info.get("task_id"))
        result["rpms"] = [
            {"available_rpm": rpm.get("available_rpm"), "result": TaskStatus.FAIL, "log": einfo}
