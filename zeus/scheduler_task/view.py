...
from zeus.config_manager.view import CollectConfig, ObjectFileConfig
...
class SyncStatus(object):
    ...
    def deal_pam_d_config(host_info, directory_path):
        # 先获取/etc/pam.d下有哪些文件
        status, content = ObjectFileConfig.object_file_config_content(host_info, directory_path)
            ...
            for info in infos:
                ...
                for file_path in DIRECTORY_FILE_PATH_LIST:
                    if info_path.find(file_path) == -1:
                        signal_file_content = {"filePath": info.get("path"), "contents": info.get("content")}
    ...
    @staticmethod
    def collect_file_infos(param, host_infos_result):
        ...
        for host_id, file_list in host_id_with_config_file.items():
            ...
            for file_path in DIRECTORY_FILE_PATH_LIST:
                if file_path in file_list:
                    ...
                    object_file_paths = SyncStatus.deal_pam_d_config(host_info, file_path)
                    if object_file_paths:
                        file_list.extend(object_file_paths)
            host_file_content_result = CollectConfig.get_file_content(host_info, file_list)
            # 处理结果
            SyncStatus.deal_host_file_content(domain_result, host_file_content_result)
    ...
    def make_database_engine():
        ...
        MysqlProxy.engine = create_database_engine(
            engine_url, configuration.mysql.get("POOL_SIZE"), configuration.mysql.get("POOL_RECYCLE"))
    ...
    def get_domain_host_ids(domain_list_resp, host_sync_proxy):
        ...
        for domain in domain_list_resp:
            ...
            status, host_sync_infos = host_sync_proxy.get_domain_host_sync_status(domain_name)
    ...
    def get_all_host_infos():
        ...
        proxy = HostProxy()
        proxy.connect()
        status, host_list = proxy.get_host_info({"username": "admin", "host_list": list()}, True)
        if status != state.SUCCEED:
            return {}
    ...
    def compare_conf(expected_confs_resp, domain_result):
        ...
        # 调用ragdoll接口比对
        try:
            request_data = {"expectedConfsResp": expected_confs_resp, "domainResult": domain_result}
            domain_diff_response = requests.post(domain_conf_diff_url, data=json.dumps(request_data), headers=headers)
    ...
    def update_sync_status_for_db(domain_diff_resp, host_sync_proxy):
        if domain_diff_resp:
            status, save_ids = host_sync_proxy.update_domain_host_sync_status(domain_diff_resp)
            update_result = sum(save_ids)
            if status != SUCCEED or update_result == 0:
                LOGGER.error("failed update host sync status data")
            if update_result > 0:
                LOGGER.info("update %s host sync status  basic info succeed", update_result)
        else:
            LOGGER.info("no host sync status data need to update")
            return

    @staticmethod
    def update_sync_status_scheduler():
        headers = {"Content-Type": "application/json"}
        # 获取所有的domain
        ...
        if not domain_list_resp:
            LOGGER.error("Failed to get all domain, please check interface /domain/queryDomain")
            return
        ...
        try:
            expected_confs_response = requests.post(domain_list_url, data=json.dumps(domain_names), headers=headers)
            expected_confs_resp = json.loads(expected_confs_response.text)
        except requests.exceptions.RequestException as connect_ex:
            LOGGER.error(f"Failed to get all domain expected conf list, an error occurred: {connect_ex}")
            return
        if not expected_confs_resp:
            LOGGER.error("Failed to get all domain confs, please check interface /confs/queryExpectedConfs")
            return

        # 方式一 创建数据引擎
        SyncStatus.make_database_engine()
        # 方式一 根据domain获取所有的id，从host_conf_sync_status表中读取
        host_sync_proxy = HostSyncProxy()
        host_sync_proxy.connect()
        domain_host_id_dict = SyncStatus.get_domain_host_ids(domain_list_resp, host_sync_proxy)
        if not domain_host_id_dict:
            LOGGER.info("no host sync status data need to update")
            return
        # 获取所有admin下面的ip的信息
        host_infos_result = SyncStatus.get_all_host_infos()
        ...
        domain_paths = {}
        SyncStatus.get_domain_files(domain_paths, expected_confs_resp)
        ...
        for domain_name, host_id_list in domain_host_id_dict.items():
            ...
            if file_paths:
                for host_id in host_id_list:
                    data_info = {
                        "host_id": host_id,
                        "config_list": file_paths
                    }
                    data["infos"].append(data_info)
            if data["infos"]:
                result = SyncStatus.collect_file_infos(data, host_infos_result)
                domain_result[domain_name] = result
        # 调用ragdoll接口进行对比
        domain_diff_resp = SyncStatus.compare_conf(expected_confs_resp, domain_result)
        # 根据结果更新数据库
        SyncStatus.update_sync_status_for_db(domain_diff_resp, host_sync_proxy)
