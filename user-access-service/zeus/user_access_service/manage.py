...
Time:
Author:
Description: Manager that start zeus user access service
"""
...
from zeus.user_access_service.urls import URLS

app = init_application(name="zeus.user_access_service", settings=configuration, register_urls=URLS)
...
def register_service():
    ...
    LOGGER.info("register user_access_service service")
    if not register_center.register_service(
        service_name="user_access_service", service_info=service_data, ephemeral=True
    ):
        raise RuntimeError("register user_access_service service failed")

    LOGGER.info("register user_access_service service success")
